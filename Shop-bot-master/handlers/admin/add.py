...
@dp.callback_query_handler(IsAdmin(), category_cb.filter(action='view'))
async def admin_category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    # Fetch the category index
    category_idx = callback_data['id']
    
    # Fetch all products linked to this category
    products = db.fetchall('SELECT * FROM products WHERE tag = (SELECT title FROM categories WHERE idx=?)', (category_idx,))
    
    if not products:
        await query.answer("No products found in this category (Admin View).", show_alert=True)
        return

    # Inform admin of the products in this category
    await query.message.edit_text(f"Here are the products in the selected category (Admin View):")
    ...
    # Add options for managing products
    await query.message.answer("Would you like to add or delete products?")
    # Add logic to display products for admin (e.g., with options to delete/edit).
